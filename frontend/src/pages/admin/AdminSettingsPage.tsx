...
const AdminSettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      try {
        ...
        // Appearance settings
        setColorTheme(data.appearance.colorTheme);
        ...
        setPasswordMinLength(data.security.passwordMinLength);
        setPasswordRequireSpecial(data.security.passwordRequireSpecial);
        
        // Email settings
        setEmailEnabled(data.email.emailEnabled);
        setEmailFromName(data.email.emailFromName);
        setEmailFromAddress(data.email.emailFromAddress);
        
        // API settings
        setApiRateLimit(data.api.apiRateLimit);
        setAllowCors(data.api.allowCors);
        
        // Notification settings
        setAdminNotifyOnNewUsers(data.notifications.adminNotifyOnNewUsers);
        setAdminNotifyOnErrors(data.notifications.adminNotifyOnErrors);
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      alert(err instanceof Error ? `Error: ${err.message}` : 'Failed to save settings');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Loading State */}
      {loading && (
        <div className="bg-white dark:bg-dark-800 rounded-xl shadow-sm p-6 flex justify-center">
